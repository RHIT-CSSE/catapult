...
class Paddle:
	...
	# Construct a paddle with a given x and y
	...
	# Draw this paddle on the given window passed in through "surface"
	...
	# Set the velocity of this paddle
...
class Star:
	...
	# Draw this Star on the given window passed in through "surface"
    def draw(self, surface):
...
class Ball:
	...
	# Draw this Ball on the given window passed in through "surface"
    def draw(self, surface):
	...
	# determine if this ball has collided with the "other" ball
...
class Planet(Ball):
	...
	# Draw this Planet on the given window passed in through "surface"
    def draw(self, surface):
...
# Check collisions between the paddle and the balls
...
# Check collisions between the balls
...
# main part of the program that runs the game	
def eventLoop():
    message = "PaddleBall!"
    startTime = time()
	...
	# The main loop of the program that repeats until the program is exited in some way
    while True:
        events = pygame.event.get()
		
		# The event loop that takes user input via events from pygame.event.get() and
		# interprets them
        for event in events:
            if event.type == QUIT:
                return
		...
		# run the rest of the game code that is independent of the user input 
		# (moves balls and checks collisions)
        if not paused:    
            screen.fill([0,0,0])
            for star in stars:
                star.draw(screen)
